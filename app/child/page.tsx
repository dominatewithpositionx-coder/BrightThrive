...
function ChildView({ child, missions, rewards, streak, mood, onBack, onMissionToggle, onGenerateMore, generatingMore, missionRound, missionPack, missionError, missionSuccess, weather, isDemoMode, isAutoGenerating, autoGenFailed, onRetryGen, supabase }: {
  ...
  async function askParent(reward: Reward) {
    ...
    try {
      ...
      const { data: insertData, error } = await supabase.from('reward_redemptions').insert({
        child_id: child.id,
        reward_id: reward.id,
      ...
      console.log('[reward insert error]', error);
      if (error) {
        console.log('[reward insert error.code]', error.code);
        console.log('[reward insert error.message]', error.message);
        console.log('[reward insert error.details]', error.details);
        console.log('[reward insert error.hint]', error.hint);
        setRequestingRewardId(null);
        return;
      }
      ...
      <div className="px-4 max-w-lg mx-auto space-y-4">
        ...
        {allDone && (
          ...
          >
            ...
            {/* ── Reward CTA — always shown inside celebration card ── */}
              ...
              {sortedRewards.length === 0 ? (
                ...
                <div className="bg-white/20 backdrop-blur-sm rounded-2xl px-4 py-3 text-center">
                  <p className="text-sm font-semibold text-white">
                    Earn <strong>{nextReward!.coin_cost - child.points} more coins</strong> to unlock {nextReward!.title} 🚀
                  </p>
                </div>
              )}
            </div>
