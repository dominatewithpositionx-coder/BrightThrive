...
type Mission = { id: string; child_id: string; is_completed: boolean; created_at: string };
...
function computeStreak(ledger: LedgerEntry[], childId: string): number {
  const days = Array.from(
    new Set(ledger.filter((h) => h.child_id === childId && h.description?.startsWith('Completed task:')).map((h) => new Date(h.created_at).toLocaleDateString()))
...
function WeeklyChart({ ledger, childId, days }: { ledger: LedgerEntry[]; childId: string; days: string[] }) {
  const pointsByDay = days.map((day) =>
    ledger.filter((h) => h.child_id === childId && h.amount > 0 && new Date(h.created_at).toLocaleDateString() === day)
      ...
      <div className="flex items-end gap-1.5 h-16">
        {pointsByDay.map((pts, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full rounded-t-sm bg-green-400 transition-all" style={{ height: `${Math.max(4, (pts / max) * 52)}px` }} title={`${pts} pts`} />
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const [{ data: childData }, { data: walletData }, { data: missionData }, { data: ledgerData }, { data: redemptionData }] = await Promise.all([
        supabase.from('children').select('id, name'),
        supabase.from('bt_coin_wallet').select('child_id, balance'),
        supabase.from('missions').select('id, child_id, is_completed, created_at'),
        supabase.from('bt_coin_ledger').select('id, child_id, amount, description, created_at').order('created_at', { ascending: false }),
        supabase.from('reward_redemptions').select('id'),
      ]);
      ...
      setRedemptionCount((redemptionData || []).length);
      setLoading(false);
    }
    fetchData();
  }, []);

  const totalTasksCompleted = missions.filter((m) => m.is_completed).length;
  const totalPointsEarned = ledger.filter((h) => h.amount > 0).reduce((sum, h) => sum + h.amount, 0);
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={Gift} label="Rewards Redeemed" value={redemptionCount} color="bg-orange-500" />
      </div>

      <div>
        <h2 className="text-lg font-semibold mb-3">Weekly Summary</h2>
        {children.length === 0 ? (
          <p className="text-gray-500 italic">No children added yet. <a href="/dashboard/children" className="text-green-600 underline">Add a child</a> to see stats here.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {children.map((child) => {
              const childLedger = ledger.filter((h) => h.child_id === child.id);
              const childCompleted = missions.filter((m) => m.child_id === child.id && m.is_completed).length;
              const childEarned = childLedger.filter((h) => h.amount > 0).reduce((sum, h) => sum + h.amount, 0);
              ...
              return (
                <div key={child.id} className="bg-white rounded-2xl border shadow-sm overflow-hidden">
                  ...
                  <div className="px-5 pb-5">
                    <WeeklyChart ledger={ledger} childId={child.id} days={days7} />
                  </div>
                </div>
              );
            })}
              ...
              <tbody>
                {recentActivity.map((entry) => (
                  <tr key={entry.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className={`px-4 py-3 font-semibold ${entry.amount > 0 ? 'text-green-600' : 'text-red-500'}`}>
